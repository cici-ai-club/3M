...
class CaptionModel(nn.Module):
    ...
    def beam_search(self, init_state, init_logprobs, *args, **kwargs):
        ...
        length_penalty = utils.penalty_builder(opt.get('length_penalty', 'avg_0'))
            ...
            for divm in range(group_size): 
                if t >= divm and t <= self.seq_length + divm - 1:
                    ...
                    if decoding_constraint and t-divm > 0:
                        logprobsf.scatter_(1, beam_seq_table[divm][t-divm-1].unsqueeze(1).cuda(), -10e20)
                    if remove_bad_endings and t-divm > 0:
                        logprobsf[torch.from_numpy(np.isin(beam_seq_table[divm][t-divm-1].cpu().numpy(), self.bad_endings_ix).astype(np.bool)), 0] = -10e20
                    # suppress UNK tokens in the decoding
                    logprobsf[:,logprobsf.size(1)-1] = logprobsf[:, logprobsf.size(1)-1] - 10e20 
                    if block_trigrams and t-divm >= 3:
                        # Store trigram generated at last step
                        prev_two_batch = beam_seq_table[divm][t-3:t-1] #time*beam_size
                        for i in range(bdash): # = seq.size(0)
                            prev_two = (prev_two_batch[0][i].item(), prev_two_batch[1][i].item())
                        ...
                        mask = torch.zeros(logprobsf.size(), requires_grad=False).cuda() # batch_size x vocab_size
                        ...
                        alpha = 10e20 # = 4
                        logprobsf = logprobsf + (mask * -0.693 * alpha) # ln(1/2) * alpha (alpha -> infty works best)
        ...
        if sample_method == 'greedy':
            ...
            if sample_method.startswith('top'): # topk sampling
                ...
                if 0 < top_num < 1:
                    ...
                    the_k = int(top_num)
                    tmp = torch.empty_like(logprobs).fill_(-10e20)
