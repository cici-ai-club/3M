...
def main(params):
    ...
    for i, img in enumerate(imgs):
        if img['image_hash'].startswith('ac8'):
            continue
#        if img['split'] == 'train' or img['split'] == 'valid' or img['split'] == 'val':
#            continue
        if '[DISCONNECT]' in img['sentences'][0]  or '[TIMEOUT]' in img['sentences'][0] or '[RETURNED]' in img['sentences'][0]:
            print(img['sentences'][0])
            continue
        out['images'].append(
            {u'id': img.get('cocoid', img['imgid']+"_"+img['personality'])})
        image_hash=img['image_hash']
        if not image_hash is None:
            img_cnt=img_cnt+1
        for j, s in enumerate(img['sentences']):
            gdindex=params['gdindex']
            if img['split']=='test':
                if gdindex!=-1:
                    if j>gdindex:
                        break
            ...
            txt = [_ if _ in wtoi else 'UNK' for _ in txt]
            s = ' '.join(txt)
            out['annotations'].append(
                    {'image_id': out['images'][-1]['id'], 'caption': s, 'id': cnt,'image_hash':image_hash})
            cnt += 1
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dict_json', default='data/personcap.json', help='output json file')
    ...
    parser.add_argument('--gdindex', default=-1, type=int,
                        help='index of which annotation to use')
